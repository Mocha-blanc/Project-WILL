...
public class Map{
	...
	public final static double BLIGHTNING=0.5; //Probabilite d'apparition de foudre
	public final static double BRAIN=0.5;//Probabilite d'apparition de pluie
	...
	public Map(int x, int y){
		...
		for (int i=1; i<x-1 ;i++) {
			for (int j=1; j<y-1 ; j++){
				...
				//}
				System.out.print((int)((Map.Get2DPerlinNoiseValue(i, j,RESOLUTION)+1)*0.5*6)+" ");
			}
			System.out.println();
		}
			...
			for (int i=1; i<x-1 ;i++) {
				for (int j=1; j<y-1 ; j++) {
					if(terrain[i][j]==VOLCANO+cpt-1 && terrain[i-1][j-1]>=VOLCANO+cpt-1 && terrain[i][j-1]>=VOLCANO+cpt-1 && terrain[i+1][j-1]>=VOLCANO+cpt-1 && terrain[i-1][j]>=VOLCANO+cpt-1 && terrain[i+1][j]>=VOLCANO+cpt-1 && terrain[i-1][j+1]>=VOLCANO+cpt-1 && terrain[i][j+1]>=VOLCANO+cpt-1 && terrain[i+1][j+1]>=VOLCANO+cpt-1) {
						tabV[i][j]+=1;
						System.out.print(tabV[i][j]+"ca rentre");
					}
				}
				System.out.println();
			}
...
//				image.affichageLava(g2,frame,i,j,lava);
//			}
//		for (int i=0; i<x ; i++)
//			for (int j=0; j<y ; j++) {
//				image.affichageBordurenord(g2,frame,i,j,terrain);
//				image.affichageBordureest(g2,frame,i,j,terrain);
//				image.affichageBorduresud(g2,frame,i,j,terrain);
//				image.affichageBordurewest(g2,frame,i,j,terrain);
...
//	}
	...
	public void step(ArrayList<Starter> agent){
		...
		stepObject(agent); //agent
		stepTree(agent);//agent
		stepEffect();
	...
	public void stepTree(ArrayList<Starter> agent){//ArrayList<Agent> agent
...
//					effect[i][j]=LIGHTNING;
	...
	public void eruption(int iteration) {
		int tab[][]=new int[x][y];
