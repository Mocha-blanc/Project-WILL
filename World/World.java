...
public class World extends JPanel implements MouseWheelListener, KeyListener{
	...
	public static final int NBPREDATEUR=0;
	public static final int SALAMECHE=10;
	public static final double BCLOUD=0.0; //Probabilité d'apparition des nuages a chaque iteration
...
//		m.affichage(g2,frame, iteration);
	...
	public static void main(String[] args) {
	    ...
	    for(int cpt=0; cpt<SALAMECHE; cpt++) {
	    	w.getGestion_agent().addStater(new Salameche(w.getMap(),"salameche"));
	    	w.getGestion_agent().addStater(new Carapuce(w.getMap(),"carapuce"));
	    }
//	    w.getGestion_agent().addEgg(new Egg(1, w.getMap(), 10,10));
		int i=0;
		while (true){
//			System.out.println(w.getGestion_agent().getEgg().get(0).getX()+" "+w.getGestion_agent().getEgg().get(0).getY());
			w.step();
			i++;
		}
	}
