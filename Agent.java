...
public abstract class Agent {
	...
	public static int detection(int x, int y, String n, int i, ArrayList<Agent> a, Map m){
		int x1=x;
		int y1=y;
		if(i<=0 || m.getTree(x1, y1).getLife()>0 || m.getTerrain(x1,y1)!=Map.SAND){ //ligne ou on indique les obstacle
			return -1;
		}
		...
		//recursion
		if(0<=detection(x1, y1 - 1,n,i-1, a, m)){
			return Agent.NORD;
		}
		if(0<=detection(x1 + 1, y1,n ,i-1, a, m)){
			return Agent.EST;
		}
		if(0<=detection(x1, y1 + 1,n ,i-1, a, m)){
			return Agent.SUD;
